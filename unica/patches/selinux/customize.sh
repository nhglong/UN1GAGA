...
CIL_LIST="$(find "$WORK_DIR/$(GET_SYSTEM_EXT)/etc/selinux/mapping" -type f -printf "%f\n" | \
                    sed '/.compat./d' | sed 's/.cil//' | sort)"
...
unset ENTRIES DUPLICATES CIL_LIST VENDOR_API_LIST PATCHED SEEN
